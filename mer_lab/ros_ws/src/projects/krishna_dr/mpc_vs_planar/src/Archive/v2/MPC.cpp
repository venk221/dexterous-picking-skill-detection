...
class FG_eval
{
    ...
    { 
        // Set default value    
        _dt = 0.1;  // in sec
        _ref_pixel_pose_x    = 170;
        _ref_pixel_pose_y    = 120; 
    ...
    void operator()(ADvector& fg, const ADvector& vars) 
    {
        ...
        //std::cout<<"fgggggggggg"<<fg.size()<<std::endl;
        fg[0] = 0;
        ...
        for (int i = 0; i < _mpc_steps - 1; i++)
        {
            ...
            fg[9] = vars[_j1_vel_start + i] - (vars[_j1_vel_start + i]+ (j1_accl_limit *_dt));
            if(vars[_j2_vel_start + i]<0){j2_accl_limit = -j2_accl_limit;}
            //std::cout<<"working "<<fg.size()<<std::endl;
            fg[11] = vars[_j2_vel_start + i+1] - (vars[_j2_vel_start + i]+ (j2_accl_limit *_dt));
            //std::cout<<"complan boy "<<fg.size()<<std::endl;
        }
...
vector<float> MPC::Solve(Eigen::VectorXf state) 
{
    ...
    //     mpc_jacobian_img << -focus_const, 0, 0, 0,-focus, (vars[_pixel_y_start + i] - c_intrinsic),
    //     0, -focus_const , 0 , focus ,0 , (c_intrinsic - vars[_pixel_x_start + i]);

    //     mpc_jacobian_robot << ((l1*cos(vars[_theta1_start + i]))+(l2*cos(vars[_theta1_start + i]+vars[_theta2_start + i]))), ((l2*cos(vars[_theta1_start + i]+vars[_theta2_start + i]))),
    //     ((l1*sin(vars[_theta1_start + i]))+(l2*sin(vars[_theta1_start + i]+vars[_theta2_start + i]))) ,((l2*sin(vars[_theta1_start + i]+vars[_theta2_start + i]))),
    //     0,0,
    //     0,0,
    //     0,0,
    //     1,1;
    //     mpc_input_vector  << vars[_j1_vel_start + i] , vars[_j2_vel_start + i];
    //     mpc_pixel_st_init << vars[_pixel_x_start + i], vars[_pixel_y_start + i];
    //     mpc_theta_st_init << vars[_theta1_start + i] , vars[_theta2_start + i];
        
    //     mpc_pixel_st_out = mpc_pixel_st_init + (((mpc_jacobian_img*mpc_jacobian_robot)*mpc_input_vector) * _dt);
    //     mpc_theta_st_out = mpc_theta_st_init + (mpc_input_vector * _dt);
    //     vars[_pixel_x_start + i + 1] = mpc_pixel_st_out[0];
    //     vars[_pixel_y_start + i + 1] = mpc_pixel_st_out[1];
    //     vars[_theta1_start + i + 1]  = mpc_theta_st_out[0];
    //     vars[_theta2_start + i + 1]  = mpc_theta_st_out[1];
