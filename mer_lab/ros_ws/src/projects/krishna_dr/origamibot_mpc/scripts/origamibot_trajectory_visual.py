...
def main(args):
    ...
    # Goal Position
    target = rospy.get_param("origami_sim/target_feature")
    goalX = target[0]
    goalY = target[1]

    img_pub = rospy.Publisher("/origamibot_mpc/vis", Image, queue_size=1)

    # Initialize subscribers
    img_sub = rospy.Subscriber("origami_vs/camera_image", Image, visCallback, queue_size = 1)
    ee_sub = rospy.Subscriber("origami_vs/features", Float64MultiArray, eePosCallback, queue_size=1)
