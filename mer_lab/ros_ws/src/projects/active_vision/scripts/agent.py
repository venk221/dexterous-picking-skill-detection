...
class AgentGB(object):
    def __init__(self, env, actor_lr=ACTOR_LR, critic_lr=CRITIC_LR, gamma=GAMMA, max_size=BUFFER_CAPACITY, 
        ...
        self.actor.compile(optimizer=opt.Adam(clipnorm=ACTOR_CLIPPING, learning_rate=actor_lr))
        self.critic.compile(optimizer=opt.Adam(clipnorm=CRITIC_CLIPPING, learning_rate=critic_lr))
        self.target_actor.compile(optimizer=opt.Adam(clipnorm=ACTOR_CLIPPING, learning_rate=actor_lr))
        self.target_critic.compile(optimizer=opt.Adam(clipnorm=CRITIC_CLIPPING, learning_rate=critic_lr))
    ...
    def _ornstein_uhlenbeck_process(self, x, theta=THETA, mu=0, dt=DT, std=2.0):
        """
    ...
    def load(self):
        ...
        self.critic.build(input_shape=[(1,52),(1,1)])
        self.critic.load_weights(f"{self.path_load}/{self.critic.net_name}.h5")
        self.target_critic.build(input_shape=[(1,52),(1,1)])
