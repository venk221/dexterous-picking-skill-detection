...
bool computeEnergyFuncCallback(encoderless_vs::energyFuncMsg::Request &req, encoderless_vs::energyFuncMsg::Response &res){
    ...
    //     // For 4 features
    //     // dSmat.row(row_count) << dSdata[itr], dSdata[itr+1], dSdata[itr+2], dSdata[itr+3];
    //     // For 5 features
    //     // dSmat.row(row_count) << dSdata[itr], dSdata[itr+1], dSdata[itr+2], dSdata[itr+3], dSdata[itr+4];
    //     // For 6 features
    ...
    Eigen::VectorXf data_vec(no_of_features);
    while(row_count < window){
        for(int i=0; i <no_of_features; i++){
            data_vec[i] = dSdata[i + (row_count*no_of_features)];
        }
        dSmat.row(row_count) << data_vec.transpose();
        row_count = row_count + 1;
    }
...
int main(int argc, char **argv){
    ...
    nh->getParam("/3linkbot/estimation/window", window); //size of estimation window
    nh->getParam("/3linkbot/estimation/epsilon", eps);

    // These are for shape based VS
    nh->getParam("/3linkbot/shape_control/no_of_features", no_of_features);
    nh->getParam("/3linkbot/shape_control/no_of_actuators", no_of_actuators);
