...
void j3velCallback(const std_msgs::Float64 &msg){
    std::ofstream j3vel_plotdata("j3vel.csv",std::ios::app);
    j3vel_plotdata<<msg.data<<"\n";
...
int main(int argc, char **argv){
    ...
    std::ofstream j3vel_plot("j3vel.csv");
    ...
    ds_plot <<"cp2 x"<<","<<"cp2 y"<< ","<<"cp3 x"<<","<<"cp3 y"<<"\n";
    ds_plot.close();
    ...
    err_plot <<"Err_cp1_x"<<","<<"Err_cp1_y"<<","<<"Err_cp2_x"<<","<<"Err_cp2_y"
             << ","<<"Err_cp3_x,"<<"Err_cp3_y,"<<"Err_cp4_x,"<<"Err_cp4_y\n";
    err_plot.close();
    ...
    ros::Subscriber j1vel_sub = n.subscribe("/3linkbot/joint1_velocity_controller/command",1,j1velCallback);
    ros::Subscriber j2vel_sub = n.subscribe("/3linkbot/joint2_velocity_controller/command",1,j2velCallback);
    ros::Subscriber j3vel_sub = n.subscribe("/3linkbot/joint3_velocity_controller/command",1,j3velCallback);
