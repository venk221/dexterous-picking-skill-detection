...
def image_callback(ros_image):
    ...
    # try:
    current_cv_image = bridge.imgmsg_to_cv2(ros_image, 'passthrough')    
    ...
    # cv2.imshow("Image", current_cv_image)
    # cv2.waitKey(20)

    # gray_img=cv2.cvtColor(current_cv_image,cv2.COLOR_BGR2GRAY) 

    # blur = cv2.GaussianBlur(gray_img,(5,5),0)
    # ret3,cv_binary = cv2.threshold(blur,0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)

    cv2.imshow("Binary", cv_binary)
    cv2.waitKey(20)
