...
class DreamInferenceROS:
    ...
    def keypoint_correspondences(self, detected_kp_projs):
        ...
        print("kp_projs_raw_good_sample", kp_projs_raw_good_sample)
        print("kp_projs_raw_good_sample", kp_positions_good_sample)
    ...
    def solve_pnp_buffer(self, candidate_kp_projs_raw, candidate_kp_positions):
        ...
        if pnp_retval:
            ...
            print("robot from cam", T_robot_from_cam)

            robot_from_cam_pos = T_robot_from_cam[:3, -1]

            print("robot from cam pos", robot_from_cam_pos)
    ...
    def image_pixels(self, camera_ext, world_coords):
        image_pix = np.matmul(np.matmul(self.camera_K, np.eye(4)), np.matmul(camera_ext, world_coords))
        return image_pix
