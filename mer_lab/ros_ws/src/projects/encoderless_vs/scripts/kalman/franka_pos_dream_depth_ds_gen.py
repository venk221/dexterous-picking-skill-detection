...
kf1 = kalmanfilter.KalmanFilter(100, 10, 10, 7, 7)
kf2 = kalmanfilter.KalmanFilter(100, 10, 10, 7, 7)
kf3 = kalmanfilter.KalmanFilter(100, 10, 10, 7, 7)
kf4 = kalmanfilter.KalmanFilter(100, 10, 10, 7, 7)
kf5 = kalmanfilter.KalmanFilter(100, 10, 10, 7, 7)
kf6 = kalmanfilter.KalmanFilter(100, 10, 10, 7, 7)
kf7 = kalmanfilter.KalmanFilter(100, 10, 10, 7, 7)
...
class DreamInferenceROS:
    def __init__(self, args, single_frame_mode=True, compute_2d_to_3d_transform=False):
        ...
        self.image_sub = rospy.Subscriber(
            image_topic, Image, self.kp_from_image, queue_size=1
        )
        self.depth_sub = rospy.Subscriber(
            depth_topic, Image, self.depth_image_cb, queue_size=1
        )
        self.bridge = CvBridge()

        #Create service for keypoints generation
        # self.dream_kp_service = rospy.Service("dream_kp_service_response", dream_img, self.dream_kp_service)
        ...
        self.control_flag_pub = rospy.Publisher(
            "/franka/control_flag", Bool, queue_size = 1
        )
    ...
    #     data = {"info": {"year":2022,"version":"0.1.0","description":"","url":"","date_created":date.today()},
    #         }
    #     with open("coco_type_data.json", "w") as filename:
    ...
    def depth_image_cb(self, img):
        if img is not None:
            self.cv_depth_image = self.bridge.imgmsg_to_cv2(img, "8UC1") 
    ...
    def first_input_estimate(self, img, key_points):
        ...
        blue = (255,0,0)
        d_img = copy.deepcopy(self.cv_depth_image)
        depth = copy.deepcopy(self.cv_depth_image)
        ...
        for i in range(len(key_points)):
                    ...
                    cv2.circle(depth,(x, y),10,blue,-1)    
                    # self.kp_json.append(x)
                    # self.kp_json.append(y)
                    # self.kp_json.append(1)
        
        self.kp_json = key_points

        cv2.imwrite("/home/merlab/Pictures/depth_dataset/"+str(self.zero)+ ".jpg", d_img)
        cv2.imwrite("/home/merlab/Pictures/dream_depth_images//"+str(self.zero)+ ".jpg", depth)
    ...
    def input_estimation(self, img, key_points, corrected_x):
        ...
        if len(key_points) == 7:
            ...
            for i in range(len(key_points)):
                    ...
                    # self.kp_json.append(y)
                    # self.kp_json.append(1)
            # print("keypoints for json in subsequent estimate", self.kp_json)
            self.kp_json = key_points
            ...
            for i in range(len(latest_corrected_x)):
                    ...
                    # self.kp_json.append(1)
            print("keypoints for json in subsequent estimate", self.kp_json)
            self.kp_json = latest_corrected_x
            
        cv2.imwrite("/home/merlab/Pictures/depth_dataset/"+str(self.k)+".jpg", d_img)    
        cv2.imwrite("/home/merlab/Pictures/dream_depth_images/"+str(self.k)+".jpg", depth)
    ...
    def kp_from_image(self, image):
        ...
        if image is not None:
            ...
            # create the json files for dataset
            
            image_file = str(self.i)+".jpg"
            id = self.i
            data = {"id":id,
                    "image":image_file,
                    "keypoints": self.kp_json.tolist()}

            json_obj = json.dumps(data, indent=4)
            filename =  "/home/merlab/Pictures/depth_dataset/"+str(self.i)+".json"
    ...
    #         kp_x.append(x)
    #         kp_y.append(y)
    #         cv2.circle(img,(x, y),10,yellow,-1)
                
    #     cv2.imwrite("dream_image_kp"+str(self.j) + ".jpg", img)
    ...
    #     # binary_image = self.bridge.cv2_to_imgmsg(cv_depth_img, "bgr8")

    #     ## This part is to use the keypoints as BSpline inputs
    #     # Fitting the Spline
    #     # tck, u_params = splprep([kp_x, kp_y], k = 3, s = 0)
    ...
    #     # print("Length of feature vector", len(cp))
    ...
    #     # # cv_img = np.zeros((480, 640, 3), dtype = "uint8") 
    ...
    #     # cv2.imwrite("dream_image"+str(self.j) + ".jpg", img)
    ...
    #     return dream_kpResponse(img, kp_resp)
        # return dream_imgResponse(binary_image, kp_resp)
