...
def controlPointCallback(cp_msg):
    ...
    global cp1x, cp1y, cp2x, cp2y, cp3x, cp3y, traj_cp1_x, traj_cp1_y, traj_cp2_x, traj_cp2_y, traj_cp3_x, traj_cp3_y
...
def visCallback(msg):
    ...
    if status == 1:
        ...
        for i in range(len(traj_cp1_x) - 1):
            ...
            cv_img = cv2.line(cv_img, start_pt1, end_pt1,(255,0,0), 1)
            cv_img = cv2.line(cv_img, start_pt2, end_pt2,(0,255,0), 1)
            cv_img = cv2.line(cv_img, start_pt3, end_pt3,(255,255,0), 1)
...
def main(args):
    ...
    goal_sub = rospy.Subscriber("franka/goal_image",Image, goalImgCallback, queue_size=1)
