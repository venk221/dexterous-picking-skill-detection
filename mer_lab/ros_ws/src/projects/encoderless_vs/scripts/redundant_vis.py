...
r = rospy.get_param("3linkbot/estimation/rate")
...
def controlPointCallback(cp_msg):
    ...
    global cp1x, cp1y, cp2x, cp2y, cp3x, cp3y, cp4x, cp4y, traj_cp1_x, traj_cp1_y, traj_cp2_x, traj_cp2_y, traj_cp3_x, traj_cp3_y, traj_cp4_x, traj_cp4_y
...
def visCallback(msg):
    ...
    # Draw control points
    cv2.circle(cv_img, (int(cp1x), int(cp1y)), 3, (255,0,0),-1)
    cv2.circle(cv_img, (int(cp2x), int(cp2y)), 3, (0,255,0),-1)
    cv2.circle(cv_img, (int(cp3x), int(cp3y)), 3, (255,255,0),-1)
    ...
    if status == 1:
        ...
        cv_img = cv2.addWeighted(cv_img, 0.5, goal_image, 0.5, 0)
        ...
        for i in range(len(traj_cp1_x) - 1):
            ...
            cv_img = cv2.line(cv_img, start_pt2, end_pt2,(0,255,0),2)
            cv_img = cv2.line(cv_img, start_pt3, end_pt3,(255,255,0),2)
            cv_img = cv2.line(cv_img, start_pt4, end_pt4,(0,255,255),2)
...
def main(args):
    ...
    goal_sub = rospy.Subscriber("vsbot/goal_image",Image, goalImgCallback, queue_size=1)
