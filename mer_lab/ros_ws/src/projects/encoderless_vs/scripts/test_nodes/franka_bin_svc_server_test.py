...
from encoderless_vs.srv import franka_bin_img, franka_bin_imgResponse
...
def franka_binary_image_service(msg):
  global bridge, l, cv_img
  # Convert ROS image to cv image
  cv_depth_img = bridge.imgmsg_to_cv2(ros_depth_img, "8UC1")

  cv2.imwrite("/home/merlab/Pictures/test_skel_new/original/img"+str(l)+".jpg", cv_img)  
  ...
  test_depth = cv_depth_img

  cv2.circle(test_depth, (int(base_center[0]), int(
        base_center[1])), 5, [255, 0, 0], -1)
  cv2.circle(test_depth, (int(ee_center[0]), int(
            ee_center[1])), 5, [0, 0, 255], -1) 

  print("Base center in depth", base_center)
  print("ee center in depth", ee_center)

  cv2.imwrite("/home/merlab/Pictures/test_skel_new/depth_marker/depth"+str(l)+".jpg", test_depth)  
