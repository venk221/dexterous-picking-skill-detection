...
class DreamInferenceROS:
    ...
    def solve_pnp_buffer(self, candidate_kp_projs_raw, candidate_kp_positions):
        ...
        self.kp_projs_raw_to_try = np.array(
        ...
        for i in range(len(self.kp_projs_raw_to_try)):
            x = np.int(self.kp_projs_raw_to_try[i][0])
            y = np.int(self.kp_projs_raw_to_try[i][1])
            cv2.circle(img,(x, y),3,(0,0,255),-1)
        
        cv2.imwrite("dream_image.jpg", img)
        ...
        pnp_retval, tvec, quat = dream.geometric_vision.solve_pnp(
            kp_positions_to_try, self.kp_projs_raw_to_try, self.camera_K
        )
    ...
    def publish_pose(self):
        ...
        print("trying pixel co-ordinates", self.kp_projs_raw_to_try)
