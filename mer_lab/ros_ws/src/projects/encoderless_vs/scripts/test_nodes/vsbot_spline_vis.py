...
def image_callback(ros_image):
    ...
    cv2.imwrite('/home/jani/pics/skel.jpg', skeleton)
    ...
    new_skel = cv2.cvtColor(skeleton, cv2.COLOR_GRAY2BGR)
    ...
    for i in range(len(points)):
      x = np.int(points[i][0])
      y = np.int(points[i][1])
      cv2.circle(new_skel,(x, y),2,(0,0,255),-1)	

    cv2.imwrite('/home/jani/pics/curve_fit.jpg', new_skel)		 
