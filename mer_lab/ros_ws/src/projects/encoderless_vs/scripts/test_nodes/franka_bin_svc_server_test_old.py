...
from encoderless_vs.srv import franka_vis_bin_img, franka_vis_bin_imgResponse
...
def franka_binary_image_service(msg):
  global bridge, l, cv_depth_img, robot_img
...
#   if ee_center and base_center:
  cv2.circle(cv_img, (int(base_center[0]), int(
        base_center[1])), 5, [255, 0, 0], -1)
  cv2.circle(cv_img, (int(ee_center[0]), int(
            ee_center[1])), 5, [0, 0, 255], -1)
 

#   r = cv2.selectROI(cv_depth_img)
#   cv_depth_img = cv_depth_img[r[1]:r[1]+r[3], r[0]:r[0]+r[2]]
#   print(max(cv_depth_img))
  
#   if ee_center[0] != 0.0 and ee_center[1] != 0.0 and base_center[0] != 0.0 and base_center[1] != 0.0:
#         cv2.circle(cv_depth_img, (int(base_center[0]), int(
#             base_center[1])), 5, [255, 255, 255], -1)
#         cv2.circle(cv_depth_img, (int(ee_center[0]), int(
#             ee_center[1])), 5, [255, 255, 255], -1)
            
#   cv2.imwrite("depth_marker"+str(l)+".jpg", cv_depth_img)
...
def marker_callback(img_msg):
    ...
    # print("Size color", cv_img.shape)

    # r = cv2.selectROI(cv_img)
    ...
    # print(marker_base_flag)
    # print(marker_ee_flag) 
...
def main(args):  
  ...
  # image pub
  # img_pub = rospy.Publisher("/franka/rgb_img", Image, queue_size = 1)
