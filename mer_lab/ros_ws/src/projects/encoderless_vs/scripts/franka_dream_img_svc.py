...
class DreamInferenceROS:
    ...
    def dream_kp_service(self, msg):
        ...
        cv_depth_img = self.cv_depth

        # r = cv2.selectROI(depth)
        # print(r[0], r[1], r[2], r[3])
        
        # depth_roi = depth[r[1]:r[1]+r[3], r[0]:r[0]+r[2]]

        r = rospy.get_param('vsbot/depth_baseline/cropper')

        cv_depth_img[r[1]:r[1]+r[3], r[0]:r[0]+r[2]] = np.where(cv_depth_img[r[1]:r[1]+r[3], r[0]:r[0]+r[2]] > 0, 0, cv_depth_img[r[1]:r[1]+r[3], r[0]:r[0]+r[2]])        
...
#         cv_depth_img = cv2.cvtColor(cv_depth_img, cv2.COLOR_GRAY2BGR)
        ...
        cv2.imwrite("dream_image_depth"+str(self.j) + ".jpg", cv_depth_img)
