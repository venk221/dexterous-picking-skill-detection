...
def set_friction_states(states):
    if states[0] == "high":    #White   pin 9
        publisher1.publish(180)
    else:
        publisher1.publish(0)

    if states[1] == "high":      #Orange pin 10
        publisher2.publish(180)
    else:
        publisher2.publish(0)

    rospy.sleep(1)

def set_friction_state(req):
    if req.data == 1.0:
...
def handle_hold_object(req):
    ...
    W.setFingerTorque()
    W.moveMotor(0, 0.2)
    #W.moveMotor(0, W.motorMax[0])
    #W.moveMotor(1, W.motorMax[1])
    # W.torqueMotor(1,0.7)
    # W.torqueMotor(4,0.7)
    W.setFingerSpeed()
    return PosCommandResponse(1)
...
if __name__ == "__main__":
    ...
    W = New_Model("/dev/ttyUSB0", 20, 21, "XM")
    publisher1 = rospy.Publisher('finger1_set_friction', UInt16, queue_size = 10)
    publisher2 = rospy.Publisher('finger2_set_friction', UInt16, queue_size=10)

    fs = rospy.Service("friction_state", PosCommand, set_friction_state)
