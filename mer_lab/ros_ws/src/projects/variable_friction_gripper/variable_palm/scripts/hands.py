...
class New_Model(OpenHand):
    ...
    def __init__(self, port="/dev/ttyUSB0", s1=1, s2=2, dyn_model="XL"):
    ...
    def close_left_finger(self, amnt=[0.3]):
        for x in range(0, len(amnt)):
            self.moveMotor(x, amnt[x])
            time.sleep(0.01)

    def close_right_finger(self, amnt=[0.3]):
        for x in range(0, len(amnt)):
            self.moveMotor(3 + x, amnt[x])
            time.sleep(0.01)
