...
class syncRW_XMHandler():
    ...
    def sync_write(self, positions, address = registerDict.X_Series["ADDR_GOAL_POSITION"], reg_size = 4 ):
        '''
        Writes values to single type of register in multiple motors, i.e. only change one thing about all the motors at a time e.g. position, velocity, etc.
    ...
    def sync_read(self, address = registerDict.X_Series["ADDR_PRESENT_POSITION"], reg_size = 4 ):
        ...
        INST = 0x82
        P1, P2 = DXL_LOBYTE(address), DXL_HIBYTE(address)
        P3,P4 = reg_size, 0x00 #used if you are reading positions/loads/ etc. Need to change according to register size
    ...
    def receive_reply(self, reg_size):
        ...
        for servo in self.servos:
            ...
            if len(single_read) == 0: #this only happens in the sync_write case
                return None
