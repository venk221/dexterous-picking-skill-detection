...
int main(int argc, char** argv) {
  ...
  if(totalTime > interceptionTime) {
    ...
    // Wait until object is underneath the gripper, stop measuring time here
    exec_time += (ros::WallTime::now().toSec() - startTime);
    ...
    moveAwayClient.call(awayReq);
    // DONE!
    // Now we calculate final execution time
    exec_time += (ros::WallTime::now().toSec() - startTime);
    ROS_INFO_STREAM("Time to execute: " << exec_time << "s");
  }
