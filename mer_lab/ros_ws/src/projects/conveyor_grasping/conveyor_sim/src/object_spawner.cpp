...
#include "conveyor_sim/SpawnTrigger.h"
...
bool spawnObject(conveyor_sim::SpawnTrigger::Request& req,
		 conveyor_sim::SpawnTrigger::Response& res) {
  if(spawn_speed == 0) {
    ...
    spawnModelMsg.request.initial_pose.position.z = z;
...
int main(int argc, char** argv) {
  ...
  spawnModelClient = nh.serviceClient<gazebo_msgs::SpawnModel>("/gazebo/spawn_urdf_model", true);

  ros::ServiceServer spawnTriggerServer = nh.advertiseService("spawn_object", spawnObject);

  if(spawn_speed != 0) {
    ROS_INFO("Continuously spawning objects");
    ...
    spawnModelMsg.request.initial_pose.position.x = x;
    spawnModelMsg.request.initial_pose.position.y = y + 0.10;
    spawnModelMsg.request.initial_pose.position.z = z + 0.2; // z-buffer is 20cm above the conveyor belt surface
    spawnModelMsg.request.initial_pose.orientation.w = 1;
    while(ros::ok()) {
      int toSpawn = rand() % urdfs.size();
      spawnModelMsg.request.model_name = models[toSpawn] + '_' + std::to_string(getID());
      spawnModelMsg.request.model_xml = urdfs[toSpawn];
      if(randOrientation)
	spawnModelMsg.request.initial_pose.orientation = randQuat();
      if(!spawnModelClient.call(spawnModelMsg))
