...
def controlPointCallback(cp_msg):
    ...
    global cp1x, cp1y, cp2x, cp2y, cp3x, cp3y, cp4x, cp4y, cp5x, cp5y, traj_cp1_x, traj_cp1_y, \
        traj_cp2_x, traj_cp2_y, traj_cp3_x, traj_cp3_y, traj_cp4_x, traj_cp4_y, traj_cp5_x, traj_cp5_y
...
def visCallback(msg):
    ...
    if status == 1:
        ...
        for i in range(len(traj_cp1_x) - 1):
            ...
            cv_img = cv2.line(cv_img, start_pt4, end_pt4,(0,255,255), 3)
            cv_img = cv2.line(cv_img, start_pt5, end_pt5,(255,0,255), 3)
