...
kf1 = KalmanFilter(100, 10, 10, 7, 7)
kf2 = KalmanFilter(100, 10, 10, 7, 7)
kf3 = KalmanFilter(100, 10, 10, 7, 7)
kf4 = KalmanFilter(100, 10, 10, 7, 7)
kf5 = KalmanFilter(100, 10, 10, 7, 7)
kf6 = KalmanFilter(100, 10, 10, 7, 7)
kf7 = KalmanFilter(100, 10, 10, 7, 7)
...
class DreamInferenceROS:
    def __init__(self, args, single_frame_mode=True, compute_2d_to_3d_transform=False):
        ...
        #     depth_image_topic, Image, self.depth_image, queue_size=1
        # )
    ...
    def kp_from_image(self, image):
        ...
        if image is not None:
            ...
            #     init_dist1_2 = np.sqrt((jt1[0]-jt2[0])**2 + (jt1[1]-jt2[1])**2)
            #     init_dist2_3 = np.sqrt((jt2[0]-jt3[0])**2 + (jt2[1]-jt3[1])**2)
            #     init_dist3_4 = np.sqrt((jt3[0]-jt4[0])**2 + (jt3[1]-jt4[1])**2)
            #     init_dist4_5 = np.sqrt((jt4[0]-jt5[0])**2 + (jt4[1]-jt5[1])**2)
            #     init_dist5_6 = np.sqrt((jt5[0]-jt6[0])**2 + (jt5[1]-jt6[1])**2)
            #     init_dist6_7 = np.sqrt((jt6[0]-jt7[0])**2 + (jt6[1]-jt7[1])**2)

            #     # joint1
            #     x1,y1 = kp[0]
            #     h1, w1 = 10, 10
            #     # initial tracking window for first joint
            #     self.track_window = (x1, y1, w1, h1)
            #     # region of interest for first feature
            #     roi = img[y1:y1 + h1, x1:x1 + w1]
            #     #apply_mask
            #     hsv_jt1 = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
            #     mask1 = cv2.inRange(hsv_jt1, np.array((0, 20, 20)), np.array((180, 250, 250)))
            #     # get histogram for hsv channel
            #     self.hist_hsv = cv2.calcHist([hsv_jt1], [0], mask1,
            #                    [180], [0, 180])
            #     # normalize the retrieved values
            #     cv2.normalize(self.hist_hsv, self.hist_hsv, 0, 255,
            #               cv2.NORM_MINMAX)
            #     # termination criteria, either 10
            #     # iteration or by at least 1 pt
            #     self.termination = (cv2.TERM_CRITERIA_EPS |
            #         cv2.TERM_CRITERIA_COUNT, 10, 1)
            #     dream_ros.cam_shift(img)

            #     # joint2
            #     x2,y2 = kp[1]
            #     h2, w2 = 10, 10
            #     # initial tracking window for second joint
            #     self.track_window = (x1, y1, w1, h1)
            #     # region of interest for feature
            #     roi = img[y1:y1 + h1, x1:x1 + w1]
            #     #apply_mask
            #     hsv_jt1 = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
            #     mask1 = cv2.inRange(hsv_jt1, np.array((0, 20, 20)), np.array((180, 250, 250)))
            ...
            # else if len(kp) < 7 and kp[0][0] ! = jt1[0] and kp[0][1] ! = jt1[1]:
            #     self.marker1 = False
            # else if len(kp) < 7 and self.marker1 
        ...
        # self.control_flag_pub.publish(self.control_flag)      

    # def depth_image(self, image):
    #     if image is not None:
    #         self.cv_depth = self.bridge.imgmsg_to_cv2(image, "8UC1") 
    #         self.control_flag = True
    #     self.control_flag_pub.publish(self.control_flag)          
