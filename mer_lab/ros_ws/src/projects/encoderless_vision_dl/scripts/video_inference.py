...
weights_path = '/home/jc-merlab/Pictures/Data/trained_models/keypointsrcnn_weights_b1_e25.pth' # need to rewrite the path code
# model = KpRcnn()
# print(type(model))
...
# If the input is the camera, pass 0 instead of the video file name
    ...
    def kalman_estimate(self, img, feature):
            ...
            cv2.imwrite("/home/jc-merlab/Pictures/Data/video_results_b1e20_kalman/out_image_"+str(self.j)+".jpg", img)                 
    ...
    def vid_inf(self):
        print("is vid inf getting called")
        ...
        while(cap.isOpened()):
          # Capture frame-by-frame
            ret, frame = cap.read()
            # cv2.imwrite("/home/jc-merlab/Pictures/Data/video_results_b1e20_kalman/out_image_" + str(i) + ".jpg", frame)
            i = i+1
            if ret == True:        
        #         img = cv2.imread(frame)
                ...
                print("original model kp", key_points)
                if len(keypoints) == 6:
                    keypoints.pop(2)

                print(len(keypoints))
                print("truncated kp", keypoints)
                ...
                # cv2.imwrite("/home/jc-merlab/Pictures/Data/video_results_02/out_image_" + str(self.i) + ".jpg", img)

                if len(keypoints) == 5 and not self.executed:
                # print("first key points input", kp)
                    self.corrected_x = self.first_input_estimate(frame, keypoints)
                    self.executed = True
                # print("Corrected_X", self.corrected_x)                       
            
                elif self.executed:
                    # print("Executed?", self.executed)               
                    self.corrected_x = self.input_estimation(frame, keypoints, self.corrected_x) 
            ...
            # cv2.imwrite("/home/jc-merlab/Pictures/Data/video_results_02/out_image_" + str(i) + ".jpg", img)
