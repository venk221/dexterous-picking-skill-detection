...
//     }
//     // std::cout << "Published Goal" << std::endl;
//     // print_fvector(goal);
// }
...
int main(int argc, char **argv){
    ...
    // ros::Publisher user_input_pub = n.advertise<std_msgs::Bool>("vsbot/user_input", 1);
    ...
    // ros::Subscriber goal_sub = n.subscribe("vsbot/cur_goal", 1, goalCallback);
...
// ----------------------------- Start Servoing ---------------------------------- 
    ...
    while(!end_flag){  
    // while (status.data == 2){ // convergence condition
        // error norm "err" is always positive
        
        // compute current error & norm
        // print_fvector(cur_features);
        // std::cout << "Goals in the loop" << std::endl;
        // print_fvector(goal);
        for(int i=0; i<no_of_features;i++){            
        ...
        std::cout<< "joint 1 vel: " <<joint_vel[0]<<std::endl;
        std::cout<< "joint 2 vel: " <<joint_vel[1]<<std::endl;
        // end of with Berk
