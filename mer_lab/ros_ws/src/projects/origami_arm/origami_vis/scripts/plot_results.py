...
def main():
    ...
    # Plot velocities
    ax2.plot(velocity[0], 'b',linewidth=1.5)
    ax2.margins(x = 0.0, y=0.0)
    # ax2.set_ylim(-0.5,0.5)
    ax2.set_ylabel('Velocity (rad/s)')
    ax2.set_xlabel('Iteration #')
    ax2.grid()
    ax2.legend(['Actuator 1'], loc='upper center', bbox_to_anchor=(0.5, -.75),
          fancybox=True, shadow=True, ncol=1)

    ax3.plot(velocity[1], 'b',linewidth=1.5)
    ax3.margins(x = 0.0, y=0.0)
    # ax3.set_ylim(-0.5,0.5)
    ax3.set_ylabel('Velocity (rad/s)')
    ax3.set_xlabel('Iteration #')
    ax3.grid()
    ax3.legend(['Actuator 2'], loc='upper center', bbox_to_anchor=(0.5, -.75),
          fancybox=True, shadow=True, ncol=1)

    ax4.plot(velocity[2], 'b',linewidth=1.5)
    ax4.margins(x = 0.0, y=0.0)
    # ax4.set_ylim(-0.5,0.5)
    ax4.set_ylabel('Velocity (rad/s)')
    ax4.set_xlabel('Iteration #')
    ax4.grid()
    ax4.legend(['Actuator 3'], loc='upper center', bbox_to_anchor=(0.5, -.75),
          fancybox=True, shadow=True, ncol=1)
    ...
    # Read ee_pose
    # ee_pose = []
    # file_name = folder_path + "ee_pose.csv"
    # with open(file_name, 'r') as csvfile:
    #     plots = csv.reader(csvfile, delimiter=',')
    #     for row in plots:
    #         ee_pose.append(row)
    
    # Plot ee_pose on image of robot
