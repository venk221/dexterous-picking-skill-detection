...
int main(int argc, char **argv){
    ...
    Eigen::MatrixXf Jhat(num_features, 2);
    ...
    while(!end_flag){
        ...
        std::cout<<"Jacobian pushed"<<std::endl;

        // compute current feature error
        for(int i=0; i<num_features; i++){
            std::cout<<"i: "<<i<<std::endl;
            std::cout<<"Current feature: "<<cur_features[i]<<std::endl;
            std::cout<<"goal feature: "<<goal[i]<<std::endl;
        ...
        velocity.data.push_back(uint(abs(v[0]+v[1])/2));
        ...
        vel_log.data.push_back((v[0] + v[1])/2);
