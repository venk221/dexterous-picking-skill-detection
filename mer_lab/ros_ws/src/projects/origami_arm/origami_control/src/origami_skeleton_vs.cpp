...
int num_modules = 0;            // number of origami modules used
...
int main(int argc, char **argv){
    ...
    std::vector<float> lam;     // Visual Servoing gain Vector
    ...
    control_mode.data = 2;      // velocity control mode
    ...
    while(!end_flag){
        ...
        int row_count = 0;
        int itr = 0;
        Eigen::VectorXf temp_row_jhat(num_actuators);
        while(row_count<num_features){
            for(int i=0; i<num_actuators; i++){
                temp_row_jhat[i] = j_hat[itr];
                itr = itr + 1;
            }
            Jhat.row(row_count) << temp_row_jhat.transpose();
        ...
        // error saturation
        Eigen::VectorXf e_sat(num_features);
        for(int i=0; i<num_features; i++){
            // if (abs(e[i]) > sat){
            //     e_sat[i] = e[i]/abs(e[i]) * sat;
            // }
            // else{
                e_sat[i] = e[i];
            // }
        ...
        // control law
        v = -Jhat_inv*(Eigen::MatrixXf(K.asDiagonal())*e_sat);
        // std::cout<< Jhat_inv<<std::endl;
        // std::cout <<std::endl;
        
        // publish velocities
        velocity.data.clear();
        for(int i=0; i<num_actuators; i++){
            velocity.data.push_back(v[i]);
        }
        ...
        // State update should occur regardless of Jacobian update
        // update state
        old_features = cur_features;
