...
void velocityWriter(const std_msgs::Int8MultiArray &msg){
    std::ofstream writeVelocity("velocity.csv", std::ios::app);  // Create file if it does not exist, otherwise append to file
    
    // Convert data to c++ vector
    std::vector<int8_t> velocity = msg.data;

    // Each element in vector is entered to a separate column
    for(std::vector<int8_t>::iterator itr = velocity.begin(); itr!=velocity.end(); ++itr){
        writeVelocity <<*itr <<",";
    }
...
int main(int argc, char ** argv){
    ...
    // Subscribe to data
    ros::Subscriber vel = n.subscribe("origami_vs/velocity", 1, velocityWriter);
