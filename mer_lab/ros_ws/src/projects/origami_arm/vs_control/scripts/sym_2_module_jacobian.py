...
#** 2nd module
q6, q7, q8, q9, qe, theta, alpha, a, d = sp.symbols("q6 q7 q8 q9 qe theta alpha a d")

m56 =  m.subs({a:0, alpha:0, d:0, theta:q6})
m67 =  m.subs({a:0, alpha:-sp.pi/2, d:q7, theta:0})
m78=  m.subs({a:0, alpha:sp.pi/2, d:0, theta:q8})
m89 =  m.subs({a:0, alpha:0, d:q9, theta:0})
m9e =  m.subs({a:0, alpha:0, d:0, theta:qe})

A2 = A*m56*m67*m78*m89*m9e
...
#** Jv
Jv1 = diff(A2[0:3, 3], q1)
Jv2 = diff(A2[0:3, 3], q2)
Jv3 = diff(A2[0:3, 3], q3)
Jv4 = diff(A2[0:3, 3], q4)
Jv5 = diff(A2[0:3, 3], q5)
Jv6 = diff(A2[0:3, 3], q6)
Jv7 = diff(A2[0:3, 3], q7)
Jv8 = diff(A2[0:3, 3], q8)
Jv9 = diff(A2[0:3, 3], q9)
Jve = diff(A2[0:3, 3], qe)
...
MatZ = m01*m12*m23*m34*m45
Jw6 = MatZ[0:3, 2]
MatZ = MatZ*m56
Jw7 = sp.Matrix([0, 0, 0])      # prismatic joint
MatZ = MatZ*m67
Jw8 = MatZ[0:3, 2]
MatZ = MatZ*m78
Jw9 = sp.Matrix([0, 0, 0])      # prismatic joint
MatZ = MatZ*m89
Jwe = MatZ[0:3, 2]

#** eqn26
J_DH = sp.Matrix([(Jv1, Jv2, Jv3, Jv4, Jv5, Jv6, Jv7, Jv8, Jv9, Jve), (Jw1, Jw2, Jw3, Jw4, Jw5, Jw6, Jw7, Jw8, Jw9, Jwe)])
np.shape(J_DH)
...
s2, kappa2, phi2 = sp.symbols('s2, kappa2, phi2')
theta2 = kappa2*s2
element6 = phi2
element7 = (s2/theta2)* tan(theta2/2)
element8 = theta2
element9 = element7
element10 = -element6

A_skp =  A2.subs({q1:element1, q2:element2, q3:element3, q4:element4, q5:element5,
                  q6:element6, q7:element7, q8:element8, q9:element9, qe:element10})
...
J_f1_3 = diff(f1, s)
J_f1_4 = diff(f1, phi2)
J_f1_5 = diff(f1, kappa2)
J_f1_6 = diff(f1, s2)

J_f1 = np.hstack((J_f1_1, J_f1_2, J_f1_3, J_f1_4, J_f1_5, J_f1_6))
...
arclength2 =  (l4 + l5 + l6)/3
curvature2 =  2 * sp.sqrt(l4**2+l5**2+l6**2-l4*l5-l5*l6-l6*l4) / (d*(l4+l5+l6))
bend_dire2 =  sp.atan((l6+l5-2*l4) / (sp.sqrt(3)*(l5-l6)))

f2 =  sp.Matrix([bend_dire, curvature, arclength, bend_dire2, curvature2, arclength2])
J_f2_1 = diff(f2, l1)
J_f2_2 = diff(f2, l2)
J_f2_3 = diff(f2, l3)
J_f2_4 = diff(f2, l4)
...
J_f2 =  np.hstack((J_f2_1, J_f2_2, J_f2_3, J_f2_4, J_f2_5, J_f2_6))
#simplify(J_f2)
np.shape(J_DH_f1)
J_DH_f2 =  J_DH_f1.subs({phi:bend_dire, kappa:curvature, s:arclength,
                         phi2:bend_dire2, kappa2:curvature2, s2:arclength2})
#simplify(J_DH_f2)
...
J_continuum.evalf(subs={l1:50, l2:30, l3:50, l4:40, l5:30, l6:50, d:40})
lambda_J_continuum = sp.lambdify((l1, l2, l3, l4, l5, l6, d), J_continuum)

import dill
dill.settings['recurse'] =  True
dill.dump(lambda_J_continuum, open("origami_2_module_Jacobian", "wb"))
