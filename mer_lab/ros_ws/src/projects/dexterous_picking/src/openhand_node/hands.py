...
import openhand_node.lib_robotis_mod as libRobotis
import time
...
class OpenHand():
	...
	def __init__(self,port,servo_ids,series="RX"):
		self.port = port
		self.dyn = libRobotis.USB2Dynamixel_Device(port)	#always only one
		...
		for servo_id in self.servo_ids:
			if series == "RX" or series =="MX":
				self.servos.append(libRobotis.Robotis_Servo(self.dyn,servo_id,series))
			else: #We will be using protocol 2 instead
				self.servos.append(libRobotis.Robotis_Servo_X(self.dyn,servo_id,series))
			...
			for i in range(num_servos):
				...
				if series == "RX":
					if (self.motorDir[i] ==1 and enc > 512) or (self.motorDir[i] == -1 and enc < 512):
						...
						input2 = input("Your encoder position for motor index " + str(i) + " may cause the motor to move backwards and break gears. We recommend you resetting the fingers to prevent gear shear, proceed? [ENTER]")
					else:
				...
				#These would then be the MX and XM motors
					...
					print( "Failsafe is incorporated to prevent gear shear in Model O")
					print( "Motor encoder postion: ", enc)
					input2 = input("As an XM Motor, we can automatically fix this issue for motor " + str(i) + ", proceed? [ENTER]")
