...
def main() :
    ...
    while not rospy.is_shutdown():
        ...
        if err == sl.ERROR_CODE.SUCCESS :
            ...
            # zed.retrieve_image(depth_image_zed, sl.VIEW.CONFIDENCE, sl.MEM.CPU)
            ...
            cv2.circle(depth_image_ocv, (1506, 490), 5, (0, 254, 0), -1)  # Draw a red point on the specified coordinate
            cv2.imshow("nsank", depth_image_ocv)
            cv2.waitKey(0)

            try:
                image_msg = bridge.cv2_to_imgmsg(image_ocv, encoding="bgra8")
                depth_msg = bridge.cv2_to_imgmsg(depth_image_ocv, encoding="32FC1")
