...
class U2net(object):
    def __init__(self):
        ...
        self.service = rospy.Service("get_grasp_points", GetGrasp, self.get_grasp_points)
    ...
    def get_grasp_points(self, request):
        ...
        depth_image_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        depth_image_gray_expanded = np.expand_dims(depth_image_gray, axis=2)
        
     
        np_rgb_image = self.bridge.imgmsg_to_cv2(img_rgb_msg, desired_encoding='bgr8')
        # Uncomment For RGBD image
        combined_image = np.concatenate([np_rgb_image, depth_image_gray_expanded], axis=2)   # Uncomment For RGBD image

        # Uncomment For RGD image (Not required)
        # r_channel = np_rgb_image[:, :, 2] #For RGD, Red channel, New
        # g_channel = np_rgb_image[:, :, 1] #For RGD, Green channel, New
        # d_channel = depth_image_gray_expanded[:, :, 0] #For RGD, Depth channel, New
        # combined_image = np.concatenate((r_channel, g_channel, d_channel), axis = 2) #For RGD, New

        self.process_image(combined_image)
    ...
    def process_image(self, image):
        ...
        # model_name = 'u2net_mixData_1Dec'
        # model_name = 'u2net_mixData_Nov22'
        # model_name = 'attu2netoutside_bce_itr_468_train_4.672474_tar_0.661065'

        # model_name = 'attu2netoutside_rgb_4.827609_tar_0.650952'
        # model_name = 'attu2netoutside_depth_bce_itr_702_train_4.612252_tar_0.670321'

        # RGBD
        # model_name = 'attu2netoutside_bce_itr_1014_train_4.680667_tar_0.650706'    
        # model_name = 'attu2netoutsidergb_bce_itr_3432_train_4.590637_tar_0.638816'    # RGBD trained with blender and real image
        model_name = 'attu2netoutsidergb_bce_itr_2340_train_4.453728_tar_0.626614'
        ...
        # model_dir = os.path.join(self.pkgdir, 'scripts', 'Attention', 'saved_models', 'u2net', model_name + '.pth')
        model_dir = os.path.join(self.pkgdir, 'scripts', 'Attention', 'saved_models', 'attu2netoutside', 'RGBD', 'Model_5_RGBD_S_1', model_name + '.pth')
...
def main(argv):
    rospy.init_node('grasp_generation_node')
